...
- `almacenamiento/almacenamiento.py` — `Almacenamiento`. One session = one directory under `datos/sesiones/<session_id>/` containing a session CSV (fixed `CSV_HEADER`, validated on `abrir_sesion`) and one `.npy` per measurement (raw ADC samples; `wfmpre` scaling params like `XINCR`/`YMULT` are stored in the CSV row, not the `.npy`, together with `acq_mode`, `numavg` and `adquisiciones_promediadas` — `ACQ:NUMACQ?` read right before `CURVE?`). Rows appended to a reopened session follow that file's own header.
...
- `conexion/monitoreo.py` — `MonitoreoConexion`. Checks the three devices on a `QTimer` in the GUI thread (5 s at rest, 1 s between measurements, paused entirely during a capture via `pausar_pings`/`reanudar_pings`), but only the laser is actually polled: each cycle calls `LaserController.ping()`, which reads the read-only `State` register and, if it fails after its retries, releases the RS-232 port and clears `conectado`. The oscilloscope is not pinged — the monitor reads the `conectado` flag, which `OsciloscopioController` clears on any VXI-11 communication error during normal use; pinging it from the GUI-thread timer would freeze the GUI for the 30 s VXI-11 timeout on a dead link. The ESP32 is judged by the freshness flag of `TempWorker.consultar()`. On any disconnect it spins up a short-lived `QThread` (`_ReconexionWorker`) to retry reconnecting without blocking the GUI. If a *critical* device (laser or oscilloscope — not the ESP32) fails to reconnect after `MAX_REINTENTOS`, it triggers `ModoSeguro`.
...
- **Config**: the three machine-specific values (`TEMP_COM_PORT`, `LASER_COM_PORT`, `OSCIL_HOST`) are resolved with precedence user JSON (`%APPDATA%/automatizacion_osciloscopio/config.json`, written by the GUI "Conexión" dialog via `app/config_usuario.py`) > `app/config_local.py` (untracked) > `app/config.py`. New code must read them through `config_usuario.obtener()` at connection time, not with `from app.config import ...` at import time; `config_usuario.aplicar()` (called in `main.py` and after saving from the dialog) keeps `app.config` and the modules that still import the names by value (`control_laser.py`, `medicion.py`) in sync so no restart is needed. `app/conexion/prueba_conexion.py` holds the standalone probes used by the dialog's "Probar conexión"; they must release everything they open (the laser DLL state is process-global).
...
- `ACQ:NUMACQ?` en el TDS5054B, medido con `herramientas/prueba_buffer.py` (trigger en Auto; falta repetirlo en Normal): `ACQ:STATE RUN` **no** reinicia el contador (264 → 267 → 490); un cambio de escala sí lo reinicia, pero con un retraso de menos de ~1 s durante el cual conserva el valor viejo. La frescura de una captura se juzga con `TestigoNumacq` (`control_osciloscopio.py`), que reconoce el reinicio como una caída del contador. El MD5 de la curva no sirve como testigo: en STOP, cambiar la escala cambia el MD5 sin adquisición nueva.
