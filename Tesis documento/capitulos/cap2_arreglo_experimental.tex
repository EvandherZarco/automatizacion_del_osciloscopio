...
adhesivo. Dos de sus paredes incorporan ventanas de cuarzo que
permiten el ingreso del haz láser sin las pérdidas ni la fluorescencia
que introduciría el acrílico. La \cref{tab:piezas_celda} resume las
...
\begin{table}[H]
  ...
  \begin{tabular}{clcc}
    ...
    3 & Pared con barreno \diameter\,12 & $76 \times 76 \times 3$ & Acrílico \\
    4 & Pared con barreno \diameter\,16 & $76 \times 76 \times 3$ & Acrílico \\
    5 & Pared con barreno \diameter\,24 & $76 \times 76 \times 3$ & Acrílico \\
...
Las tres paredes transversales de la celda incorporan barrenos de
\diameter\,12, \diameter\,16 y \diameter\,24\,\si{\milli\meter}
respectivamente, cuya función es alojar y sostener el hidrófono en
posición fija dentro del líquido. El hidrófono empleado es el
desarrollado por Rangel Mendieta \cite{Rangel2023}, cuyas
características se detallan en la \cref{sec:equipos}.

El montaje mediante barrenos pasantes garantiza que la posición
...
\diameter\,10.5\,\si{\milli\meter} y \SI{1.00}{\milli\meter} de
...
\begin{table}[H]
  ...
  \begin{tabular}{ll}
    ...
    Dimensiones del elemento sensor   & \diameter\,10.5 $\times$ \SI{1.00}{\milli\meter} \\
...
El módulo expone como parámetros ajustables desde la interfaz gráfica
la longitud de registro (\texttt{HOR:RECLENGTH}), la escala vertical
(\texttt{CH$n$:SCALE}), la escala horizontal
(\texttt{HORizontal:SCAle}), el acoplamiento de entrada
(\texttt{CH$n$:COUPling}), el nivel de disparo
(\texttt{TRIGger:MAIn:LEVel}), el modo de adquisición
(\texttt{ACQ:MODE}) y el número de promedios (\texttt{ACQ:NUMAVG}).

La longitud de registro admite únicamente un conjunto discreto de
valores determinado por el instrumento, por lo que la interfaz
presenta dichos valores como opciones cerradas en lugar de aceptar
entrada numérica libre.
...
Los sensores se conectan a un bus 1-Wire compartido sobre una única
línea de datos, con alimentación externa explícita y una resistencia
de polarización de \SI{4.7}{\kilo\ohm}.

La comunicación con la computadora de control se realiza por enlace
serial a \SI{115200}{baud}. El microcontrolador transmite
periódicamente una trama de texto delimitada por comas que contiene
la temperatura promedio y el estado de presencia de cada uno de los
cuatro sensores, y acepta un conjunto reducido de comandos que
permiten verificar la comunicación y suspender o reanudar la
transmisión.

\subsection*{Consideraciones de firmware}

La adquisición de los cuatro sensores incorpora tres decisiones que
merecen mención.
...
La segunda es el direccionamiento de los sensores. La enumeración por
índice sobre el bus 1-Wire obliga a repetir el recorrido completo de
identificación en cada lectura, lo que introduce latencia y hace que
la correspondencia entre índice y sensor físico dependa del orden de
respuesta en el bus. El firmware emplea en su lugar las direcciones
únicas de cada sensor, obtenidas en una enumeración inicial, lo que
garantiza que cada lectura corresponda invariablemente al mismo
sensor físico.

La tercera es el manejo del tiempo de conversión. A resolución de
...
uno de los tres restantes respecto de él. Las constantes de
corrección resultantes se incorporan al firmware, de modo que la
compensación se aplique antes del promediado.
...
%   \caption{Diagrama de conexión del bus 1-Wire y los cuatro sensores.}
